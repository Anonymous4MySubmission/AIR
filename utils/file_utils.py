...
def given_dir_get_img(dir_path, img_extensions={'bmp', 'jpg', 'jpeg', 'pgm', 'png', 'ppm', 'tif', 'tiff', 'webp'}):

    path = pathlib.Path(dir_path)
    files = sorted([file for ext in img_extensions
...
def save_images(images: torch.Tensor, output_dir: str, file_prefix: str, nrows: int, iteration: int) -> None:
    utils.save_image(
        transforms.Normalize(mean=[-1.0, -1.0, -1.0], std=[2.0, 2.0, 2.0])(images),
